...
namespace sff
{
	...
	valueBar::valueBar(sf::Vector2f barSize, sf::Vector2f BarPos, sf::Color frontBarColor, bool drawBbar, bool drawFbar, int maxP)
	{
		...
		frontBar.setSize(sf::Vector2f((float)progress, barSize.y));
	...
	void valueBar::setPosition(sf::Vector2f BarPos)
	{
		frontBar.setPosition(BarPos);
		backBar.setPosition(BarPos);
	}
	...
	void valueBar::setScale(sf::Vector2f BarScale)
	{
		frontBar.setScale(BarScale);
		backBar.setScale(BarScale);
	}
	...
	void valueBar::render(sf::RenderWindow* win)
	{
		win->draw(backBar);
		win->draw(frontBar);
	}
